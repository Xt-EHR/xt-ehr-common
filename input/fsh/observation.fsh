...
* status 1..1 CodeableConcept "Status" """The status of the result.""" // no "result value"
...
* observationResult 1..1 Base "Observation result" """Result of the observation including text, quantity, coded and boolean results of the measurement and measurement uncertainty. Content of the observation result will vary according to the type of the observation."""
  * value[x] 1..1 string or Quantity or CodeableConcept or boolean "Value or result of the observation" 
  * uncertainty 0..1 Quantity "Uncertainty" """Measurement uncertainty type and interval should be provided if needed."""
  * dataAbsentReason 0..1 CodeableConcept "dataAbsentReason" """Provides a reason why the expected value in the element Observation.value[x] is missing."""
    ...
    * ^binding.strength = #preferred
  * referenceRange 0..* Base "Reference range" """Reference range, multiple reference ranges of different types could by providedProvides guide for interpretation of result. Reference ranges are usually implied only for a numeric scale type. Use of the same units for reference range and value is implied."""
...
* hasMember 0..* Reference(EHDSObservation) "Has member" """This observation is a group observation (e.g. a battery, a panel of tests, a set of vital sign measurements) that includes the target as a member of the group."""
...
//* subject 1..1 Reference (EHDSPatient or EHDSLocation or EHDSDevice or EHDSOrganisation or EHDSHealthProfessional or EHDSProcedure) "Subject" """The patient, or group of patients, location, device, organization, procedure or practitioner this observation is about."""
