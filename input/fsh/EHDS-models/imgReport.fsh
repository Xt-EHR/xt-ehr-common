...
* header 1..1
  ...
  * accessionNumber 0..1 string "Accession number - an identifier, managed by the RIS at the local level, which usually uniquely identifies an imaging procedure request, and links it to imaging study(ies) and related imaging report(s). As it is prefixed by the registration authority it is a globally unique ID, usable both nationally and cross-border."
  * healthInsuranceAndPaymentInformation 0..* EHDSCoverage "Health insurance and payment information" """Health insurance information is not always required, however, in some jurisdictions, the insurance number is also used as the patient identifier. It is necessary not just for identification but also forms access to funding for care."""
  * intendedRecipient[x] 0..* EHDSPatient or EHDSRelatedPerson or EHDSHealthProfessional or EHDSOrganization or EHDSDevice "Information recipient (intended recipient or recipients of the report, additional recipients might be identified by the ordering party, e.g. GP, other specialist), if applicable"

* body 1..1 Base "Imaging Report structured body"
  * orderInformation 0..* Base "B.1.1 - Order information" """Order Information (Imaging Result Report could respond to multiple service orders)"""
    * orderId 1..* Identifier "B.1.1.1 - Order Id" """An identifier of the imaging service order. Imaging Result Report may respond to multiple orders."""
    ...
    * orderReason[x] 0..* EHDSCondition or EHDSObservation "B.1.1.5 - Order reason" """An explanation or justification for why this service is being requested in coded form. """
    * clinicalQuestion 0..1 string "B.1.1.6 - Clinical question" """Specification of clinical question (goal of the investigation) to be answered by the imaging investigation."""
    * supportingInformation 0..* Base "B.1.1.7 - Supporting information" """Additional clinical information about the patient or specimen that may affect service delivery or interpretation. This information includes diagnosis, clinical findings and other observations. This includes observations explicitly requested by the producer (filler) to provide context or supporting information needed to complete the order.  For example, reporting the metal implants present in patien's body."""
      ...
      * medicationAdministration 0..* EHDSMedicationAdministration "B.1.1.7.3 - Medication Administration" """Medication administered before ordering the service."""
      ...
      * gestationalAge 0..1 Quantity "B.1.1.7.6 - Gestational age" """Duration of the pregnancy on the day on which the patient was asked or at the delivery. The duration can be given in weeks (wk) and/or days (d)."""
  ...
  * exposureInformation 0..1 Base "B.1.4 - Exposure information" """Information on total exposure to ionising radiation."""
    * effectiveDose 0..* Quantity "B.1.4.1 - Effective dose" """Sum of equivalent doses to all organs, from external and/or internal exposures, each adjusted to account for the sensitivity of the organ to radiation. [mSv]"""
      ...
      * ^binding.strength = #preferred
    * equivalentDoseInformation 1..1 Base "B.1.4.2 - Equivalent dose information" """Equivalent dose is based on the absorbed dose per organ, adjusted to account for the effectiveness of the type of radiation [mSv]."""
  ...
  * examinationReport 1..1 Base "B.1.5 - Examination Report" """Examination Report content."""
    * modality 1..* CodeableConcept "B.1.5.1 - Modality" """Imaging modality (or modalities) used during imaging investigation (DICOM CID029)."""
      ...
      * ^binding.strength = #preferred
    * bodyPart 0..* Base "B.1.5.2 - Body part" """All body parts investigated"""
    ...
    * resultData 0..1 Base "B.1.5.6 - Result data" """Laboratory report result data"""
      * resultText 1..1 string "B.1.5.6.1 - Result text" """Comments and narrative representation of the observation results and findings."""
    ...
    * conclusion 1..1 Base "B.1.5.7 - Conclusion" """A concise and clinically contextualised summary including interpretation/impression of the diagnostic report."""
      * impression 1..1 string "B.1.5.7.1 - Impression" """Narrative description of the clinical conclusion (impression)."""
  ...
  * recommendation 0..1 Base "B.1.6 - Recommendation" """Recommendations for additional imaging tests or other actions"""
  ...
  * comparisonStudy 0..* EHDSImagingReport "B.1.7 - Comparison Study" """Documentation (reference) of a prior Imaging Report to which the current images were compared."""
