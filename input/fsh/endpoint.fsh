...
* connectionType 1..* CodeableConcept "C.33.3 - Connection type" """A coded value that represents the technical details of the usage of this endpoint, such as what WSDLs should be used in what way. (e.g. XDS.b/DICOM/cds-hook)."""
...
* payload 0..* Base "C.33.6 - Payload" """The set of payloads that are provided/available at this endpoint."""
  * type 0..* CodeableConcept "C.33.6.1 - Type" """The type of content that may be used at this endpoint (e.g. XDS Discharge summaries)."""
    ...
    * ^binding.strength = #preferred
  * mimeType 0..* CodeableConcept "C.33.6.2 - Mime type" """The mime type to send the payload in - e.g. application/fhir+xml, application/fhir+json. """
    * ^binding.description = "BCP-13"
    * ^binding.strength = #preferred
  * profileCanonical 0..* uri "C.33.6.3 - Profile Canonical" """The FHIR profile that is expected at this endpoint. It describes the resources that are handled, or even simply which resource types."""
  * profileUri 0..* uri "C.33.6.4 - Profile uri" """The profile (as a uri) that is expected at this endpoint when not represented using a FHIR profile."""
* address 1..1 uri "C.33.7 - Address" """The uri that describes the actual end-point to connect to."""
* header 0..* string "C.33.8 - Header" """Additional headers / information to send as part of the notification. Usage depends on the channel type."""